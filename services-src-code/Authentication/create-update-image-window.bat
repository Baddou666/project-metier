...
docker images | findstr "token-manager"
...
docker images | findstr "token-manager" >nul 2>&1
if %errorlevel% neq 0 (
    echo [ERREUR] Aucune image token-manager trouvee localement. Faites un build d'abord.
    pause
    exit /b
)

echo Image token-managertrouvee localement.
goto choose_version
...
if "%VERSION%"=="" (
    ...
    echo 1. Test "ghcr.io/baddou666/projet-metier/test/token-manager"
    echo 2. Final "ghcr.io/baddou666/projet-metier/final/token-manager"
    echo.
...
if "%choice%"=="1" (
    set BASE_TAG=ghcr.io/baddou666/projet-metier/test/token-manager
    echo Version Test selectionnee.
) else if "%choice%"=="2" (
    set BASE_TAG=ghcr.io/baddou666/projet-metier/final/token-manager
...
docker tag token-manager:latest %IMAGE_TAG%
