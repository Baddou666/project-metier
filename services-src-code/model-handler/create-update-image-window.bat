...
docker images | findstr "model-handler"
...
docker images | findstr "model-handler" >nul 2>&1
if %errorlevel% neq 0 (
    echo [ERREUR] Aucune image model-handler trouvee localement. Faites un build d'abord.
    pause
    exit /b
)

echo Image model-handler trouvee localement.
goto choose_version
...
if "%VERSION%"=="" (
    ...
    echo 1. Test "ghcr.io/baddou666/projet-metier/test/model-handler"
    echo 2. Final "ghcr.io/baddou666/projet-metier/final/model-handler"
    echo.
...
if "%choice%"=="1" (
    set BASE_TAG=ghcr.io/baddou666/projet-metier/test/model-handler
    echo Version Test selectionnee.
) else if "%choice%"=="2" (
    set BASE_TAG=ghcr.io/baddou666/projet-metier/final/model-handler
...
docker tag model-handler:latest %IMAGE_TAG%
