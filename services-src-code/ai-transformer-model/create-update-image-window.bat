...
docker images | findstr "transformer-model"
...
docker images | findstr "transformer-model" >nul 2>&1
if %errorlevel% neq 0 (
    echo [ERREUR] Aucune image transformer-model trouvee localement. Faites un build d'abord.
    pause
    exit /b
)

echo Image transformer-model trouvee localement.
goto choose_version
...
if "%VERSION%"=="" (
    ...
    echo 1. Test "ghcr.io/baddou666/projet-metier/test/transformer-model"
    echo 2. Final "ghcr.io/baddou666/projet-metier/final/transformer-model"
    echo.
...
if "%choice%"=="1" (
    set BASE_TAG=ghcr.io/baddou666/projet-metier/test/transformer-model
    echo Version Test selectionnee.
) else if "%choice%"=="2" (
    set BASE_TAG=ghcr.io/baddou666/projet-metier/final/transformer-model
...
docker tag transformer-model:latest %IMAGE_TAG%
